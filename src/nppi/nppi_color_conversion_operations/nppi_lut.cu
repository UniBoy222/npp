...
__global__ void nppiLUT_Linear_16u_C1R_kernel(const Npp16u *pSrc, int nSrcStep, Npp16u *pDst, int nDstStep, int width,
  ...
  if (x < width && y < height) {
    ...
    int input = src_row[x];
    int segment = findLUTSegment(input, pLevels, nLevels);
    int result = linearInterpolate(input, pLevels[segment], pLevels[segment + 1], pValues[segment], pValues[segment + 1]);
...
// Kernel for 16-bit unsigned three channel linear LUT
__global__ void nppiLUT_Linear_16u_C3R_kernel(const Npp16u *pSrc, int nSrcStep, Npp16u *pDst, int nDstStep, int width,
                                              int height, const Npp32s *const *pValues, const Npp32s *const *pLevels,
                                              const int *nLevels) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < width && y < height) {
    ...
    for (int c = 0; c < 3; c++) {
      int input = src_row[src_idx + c];
      int segment = findLUTSegment(input, pLevels[c], nLevels[c]);
      int result = linearInterpolate(input, pLevels[c][segment], pLevels[c][segment + 1], pValues[c][segment], pValues[c][segment + 1]);

      result = max(0, min(65535, result));
      dst_row[dst_idx + c] = (Npp16u)result;
    }
  }
}
...
NppStatus nppiLUT_Linear_16u_C3R_Ctx_impl(const Npp16u *pSrc, int nSrcStep, Npp16u *pDst, int nDstStep, NppiSize oSizeROI,
  ...
  nppiLUT_Linear_16u_C3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
      pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height, d_pValuesPtr, d_pLevelsPtr, d_nLevels);
...
// Kernel for 16-bit signed single channel linear LUT
__global__ void nppiLUT_Linear_16s_C1R_kernel(const Npp16s *pSrc, int nSrcStep, Npp16s *pDst, int nDstStep, int width,
                                              int height, const Npp32s *pValues, const Npp32s *pLevels, int nLevels) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < width && y < height) {
    ...
    dst_row[x] = (Npp16s)result;
  }
}

// Kernel for 16-bit signed three channel linear LUT
__global__ void nppiLUT_Linear_16s_C3R_kernel(const Npp16s *pSrc, int nSrcStep, Npp16s *pDst, int nDstStep, int width,
                                              int height, const Npp32s *const *pValues, const Npp32s *const *pLevels,
                                              const int *nLevels) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < width && y < height) {
    ...
    for (int c = 0; c < 3; c++) {
      ...
      result = max(-32768, min(32767, result));
      dst_row[dst_idx + c] = (Npp16s)result;
    }
  }
}
...
NppStatus nppiLUT_Linear_16s_C1R_Ctx_impl(const Npp16s *pSrc, int nSrcStep, Npp16s *pDst, int nDstStep, NppiSize oSizeROI,
  ...
  nppiLUT_Linear_16s_C1R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
      pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height, d_pValues, d_pLevels, nLevels);
...
NppStatus nppiLUT_Linear_16s_C3R_Ctx_impl(const Npp16s *pSrc, int nSrcStep, Npp16s *pDst, int nDstStep, NppiSize oSizeROI,
  ...
  nppiLUT_Linear_16s_C3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
      pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height, d_pValuesPtr, d_pLevelsPtr, d_nLevels);
...
// Kernel for 32-bit float single channel linear LUT
__global__ void nppiLUT_Linear_32f_C1R_kernel(const Npp32f *pSrc, int nSrcStep, Npp32f *pDst, int nDstStep, int width,
                                              int height, const Npp32s *pValues, const Npp32s *pLevels, int nLevels) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < width && y < height) {
    ...
    int input = (int)src_row[x];
    int segment = findLUTSegment(input, pLevels, nLevels);
    int result = linearInterpolate(input, pLevels[segment], pLevels[segment + 1], pValues[segment], pValues[segment + 1]);

    dst_row[x] = (Npp32f)result;
  }
}

// Kernel for 32-bit float three channel linear LUT
__global__ void nppiLUT_Linear_32f_C3R_kernel(const Npp32f *pSrc, int nSrcStep, Npp32f *pDst, int nDstStep, int width,
                                              int height, const Npp32s *const *pValues, const Npp32s *const *pLevels,
                                              const int *nLevels) {
  int x = blockIdx.x * blockDim.x + threadIdx.x;
  int y = blockIdx.y * blockDim.y + threadIdx.y;
  ...
  if (x < width && y < height) {
    ...
    for (int c = 0; c < 3; c++) {
      int input = (int)src_row[src_idx + c];
      int segment = findLUTSegment(input, pLevels[c], nLevels[c]);
      int result = linearInterpolate(input, pLevels[c][segment], pLevels[c][segment + 1], pValues[c][segment], pValues[c][segment + 1]);

      dst_row[dst_idx + c] = (Npp32f)result;
    }
  }
}
...
NppStatus nppiLUT_Linear_32f_C1R_Ctx_impl(const Npp32f *pSrc, int nSrcStep, Npp32f *pDst, int nDstStep, NppiSize oSizeROI,
  ...
  nppiLUT_Linear_32f_C1R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
      pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height, d_pValues, d_pLevels, nLevels);
...
NppStatus nppiLUT_Linear_32f_C3R_Ctx_impl(const Npp32f *pSrc, int nSrcStep, Npp32f *pDst, int nDstStep, NppiSize oSizeROI,
  ...
  nppiLUT_Linear_32f_C3R_kernel<<<gridSize, blockSize, 0, nppStreamCtx.hStream>>>(
      pSrc, nSrcStep, pDst, nDstStep, oSizeROI.width, oSizeROI.height, d_pValuesPtr, d_pLevelsPtr, d_nLevels);
