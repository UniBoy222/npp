...
__global__ void nppiHistogramEven_8u_C1R_kernel_shared(const Npp8u *pSrc, int nSrcStep, int width, int height,
  ...
  if (x < width && y < height) {
    ...
    // Note: NPP histogram bins are [nLevels-1] in size, representing nLevels boundaries
    // Pixel values in [nLowerLevel, nUpperLevel) are mapped to bins [0, nLevels-2]
    if (pixel_value >= nLowerLevel && pixel_value < nUpperLevel) {
      int range = nUpperLevel - nLowerLevel;
      // Use long long to avoid integer overflow for large values
      long long bin = ((long long)(pixel_value - nLowerLevel) * (nLevels - 1)) / range;
      // Clamp to valid range [0, nLevels-2]
      if (bin >= nLevels - 1) bin = nLevels - 2;
...
// Multi-channel histogram kernels - 8u C4R
